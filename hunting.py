...
sensor.set_vflip(True) # 这两者是翻转，当出现小车向反方向转动可以整整
...
while(True):
    ...
    deflection_angle = math.degrees(deflection_angle) * 0.7
    #将计算结果的弧度值转化为角度值
    ...
    if(deflection_angle<0):
        is_turn = 1
    # 获取个位数
    ones_place = int(abs(deflection_angle)) % 10
    # 获取十位数
    tens_place = int(abs(deflection_angle) // 10) % 10
